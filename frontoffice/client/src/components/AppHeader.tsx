 ...
 * s'ils sont réellement alimentés par une route de l'API — matchs
 * (`/matches`) et notifications non lues (`/notifications`). Pas de pastille
...
export default function AppHeader({ initial }: { initial?: string }) {
  ...
  useEffect(() => {
    ...
    // 401 et le visiteur n'a de toute façon ni match ni notification.
    if (!signedIn) { setMatches(0); setUnread(0); return }
    let alive = true
    discoveryApi
      .matches()
